...
def process(nodes, dropout, regularizers, lr, momentum, decay, mynesterov, myloss, epochs, batchsize, filename, modelname):
    nodes=environ['nodes']
    dropout=environ['dropout']
    regularizers=environ['regularizers']
    lr=environ['lr']
    momentum=environ['momentum']
    decay=environ['decay']
    mynesterov=environ['mynesterov']
    myloss=environ['myloss']
    epochs=environ['epochs']
    batchsize=environ['batchsize']
    filename=environ['filename']
    modelname=environ['modelname']  
...
# In[ ]:
    ...
    model.add(layers.Dense(nodes, input_dim=2048, activation='relu', kernel_regularizer=keras.regularizers.l2(l=regularizers)))
    ...
    sgd=SGD(lr=learningrate, momentum=momentum, decay=decay, nesterov=mynesterov)
